...
struct Matrix3
{
	...
	constexpr explicit Matrix3(const TupleType& t) noexcept;
...
struct alignas(16) Matrix3<float>
{
	...
	explicit Matrix3(const SimdTupleType& t) noexcept;
...
template<typename T>
	requires std::floating_point<T>
inline Matrix3<T> inverseTranspose(const Matrix3<T>& m) noexcept
{
	return Matrix3<T>(Uninitialized()).setInverseTranspose(m);
}

template<typename T>
	requires std::floating_point<T>
inline Matrix3<T> inverseTranspose(Matrix3<T>&& m) noexcept
{
	m.invert();
	m.transpose();
	return m;
