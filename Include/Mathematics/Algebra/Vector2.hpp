...
struct Vector2<T>
{
	...
	bool approxEquals(const Vector2& v) const noexcept;
	bool approxEquals(const Vector2& v, T tolerance) const noexcept;
...
struct alignas(16) Vector2<float>
{
	...
	float getMagnitude() const noexcept { return simd::extract(simd::sqrt1(simd::dot2(xy, xy))); }
	float getMagnitudeSquared() const noexcept { return simd::extract(simd::dot2(xy, xy)); }
...
{
	size_t operator()(const ::mathematics::templates::Vector2<T>& v) const noexcept
	{
		hash<T> hasher;
...
template<>
struct hash<::mathematics::templates::Vector2<float>>
{
	size_t operator()(const ::mathematics::templates::Vector2<float>& v) const noexcept
	{
		return hash<typename ::simd::float4>()(v.xy);
	}
};
